...
BASE_IMAGE = "ismailbouajaja/cuda-python"
...
TAG_FORMAT = "{CUDA_TAG}-python{PYTHON_VERSION}-poetry{POETRY_VERSION}"
...
def get_tag(
    ...
    poetry_version: str = POETRY_VERSION[-1]
    ) -> str:
...
def update_github(
    ...
    latest_poetry_version: str = POETRY_VERSION[-1],
    cwd: str = CWD,
    verbose: int = 1
    ) -> None:
    try:
...
def publish_images(
    ...
    def build_func(piece, index, data):
        cuda_version, cuda_cudnn_option, cuda_type, cuda_os_option, python_version, poetry_version = piece
    ...
    parallelize(
        build_func,
        list(product(cuda_versions, cuda_cudnn_options, cuda_types, cuda_os_options, python_versions, poetry_versions)),
        verbose=verbose,
        n_jobs=n_jobs
    )
